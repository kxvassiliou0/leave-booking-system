...
describe('JobRoleService.getAll', () => {
  it('returns all job roles from the repository', async () => {
    // Arrange
    const jobRoles = [makeJobRole(), makeJobRole({ id: 2, name: 'Senior Contractor' })]
...
describe('JobRoleService.delete', () => {
  ...
  it('throws CONFLICT AppError on foreign key constraint violation', async () => {
    // Arrange
    mockRepo.delete.mockRejectedValue(new Error('foreign key constraint fails'))

    // Act & Assert
    await expect(service.delete(1)).rejects.toThrow(
      new AppError('Cannot delete job role: one or more users are assigned to it', StatusCodes.CONFLICT)
    )
  })
})
