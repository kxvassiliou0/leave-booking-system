...
export class PublicHolidayController {
  ...
  getById = async (req: Request & { params: { id: string } }, res: Response): Promise<void> => {
    const id = parseInt(req.params.id, 10)
    if (isNaN(id)) {
      ResponseHandler.sendErrorResponse(res, StatusCodes.BAD_REQUEST, 'Invalid ID')
      return
    }
    try {
  ...
  create = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!date || !name) {
      ResponseHandler.sendErrorResponse(res, StatusCodes.BAD_REQUEST, 'date and name are required')
      return
    }
    try {
  ...
  update = async (req: Request & { params: { id: string } }, res: Response): Promise<void> => {
    const id = parseInt(req.params.id, 10)
    if (isNaN(id)) {
      ResponseHandler.sendErrorResponse(res, StatusCodes.BAD_REQUEST, 'Invalid ID')
      return
    ...
    try {
      const holiday = await this.service.update(id, req.body as { date?: string; name?: string })
      res.status(StatusCodes.OK).json(holiday)
    } catch (err) {
      this.handleError(res, err)
    }
  ...
  delete = async (req: Request & { params: { id: string } }, res: Response): Promise<void> => {
    const id = parseInt(req.params.id, 10)
    if (isNaN(id)) {
      ResponseHandler.sendErrorResponse(res, StatusCodes.BAD_REQUEST, 'Invalid ID')
      return
