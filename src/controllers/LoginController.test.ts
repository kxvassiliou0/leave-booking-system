...
describe('LoginController', () => {
  ...
  it('login — user not found returns 401', async () => {
    // Arrange
    mockService.login.mockRejectedValue(new AppError('User not found', StatusCodes.UNAUTHORIZED))
    const req = mockRequest({}, { email: 'nobody@company.com', password: 'Password123!' })
    const res = mockResponse()
  ...
  it('login — incorrect password returns 401', async () => {
    // Arrange
    mockService.login.mockRejectedValue(new AppError('Incorrect password', StatusCodes.UNAUTHORIZED))
